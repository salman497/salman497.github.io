...
import { addQueryParam, getQueryParam } from '../utils/basic-utils';
@Injectable()
    ...
    () =>
      this.actions$.pipe(
        ...
        map((action: RouterNavigationAction) => {
          // let linkId = this.activatedRoute.snapshot.queryParamMap.get('linkId'); 
          // let linkId = getQueryParam('linkId');
          // if(!linkId) {
          //   linkId = uuidv4(); 
          // }
